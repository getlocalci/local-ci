...
export function activate(context: vscode.ExtensionContext): void {
	...
	vscode.commands.registerCommand('localCi.refresh', () => ciProvider.refresh());
	...
	const runActionDisposable = vscode.commands.registerCommand('localci.runAction', async ( jobName ) => {
		...
		const setDelay = (milliseconds: number) => new Promise(resolve => setTimeout(resolve, milliseconds));
		...
		const finalTerminal = vscode.window.createTerminal({
			...
			message: 'This is the final state inside the container before it exited',
			hideFromUser: true,
		});
		finalTerminal.show();

		const interval = setInterval( () => {
			finalTerminal.sendText(`docker commit $( docker ps -lq ) local-ci-${ jobName }`);
		}, 5000 );

		vscode.window.onDidCloseTerminal(t => {
			clearTimeout( interval );
			if (t.exitStatus && t.exitStatus.code) {
				vscode.window.showInformationMessage(`The job ${ jobName } failed, opening debugger shell` );
